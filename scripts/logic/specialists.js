...
async function applyDelta(actor, key, delta, { min = 0, max = null } = {}) {
  ...
  const rawMax = max ?? actor.getFlag(FLAG_SCOPE, `${key}Max`);
  const parsedMax = rawMax == null ? null : Number(rawMax);
  const targetMax = Number.isFinite(parsedMax) ? parsedMax : null;
